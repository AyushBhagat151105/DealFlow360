    ...
    <main className="min-h-full overflow-y-auto bg-background">
      <div className="mx-auto max-w-7xl space-y-6 p-6">
        <header className="flex flex-wrap items-end justify-between gap-4 border-b border-border pb-4">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              {hasManualEdits && (
                <Badge className="bg-highlighter-yellow/50 text-forest-ink border-highlighter-yellow/60 text-[10px]">
                  Manual Override Active
                </Badge>
              )}
            </div>
            <h1 className="text-2xl font-semibold tracking-[-0.02em] text-foreground">Warehouse Allocation Split</h1>
          ...
          <div className="flex items-center gap-2">
            {hasManualEdits && (
              <Button
                ...
                className="gap-1.5 text-xs border-pencil-gray text-forest-ink hover:bg-whisper-gray"
              >
        ...
        <section className="grid gap-px border border-border bg-border sm:grid-cols-4">
          ...
          />
          <Metric
            icon={<AlertTriangle className={`h-4 w-4 ${hasBackorders ? "text-terracotta" : "text-muted-foreground"}`} />}
            label="Backorders"
        ...
        {hasBackorders && (
          <div className="p-4 rounded-lg bg-highlighter-yellow/20 border border-highlighter-yellow/60 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <AlertTriangle className="h-5 w-5 text-forest-ink shrink-0" />
              <div>
                <p className="text-sm font-semibold text-forest-ink">Backorder Detected ({totalBackorders} Units)</p>
            ...
            <Button
              variant="outline"
              size="sm"
              className="border-pencil-gray text-forest-ink hover:bg-whisper-gray shrink-0 text-xs"
        ...
        <Card className="rounded-xl border border-pencil-gray/40 bg-card shadow-none">
          <CardHeader className="border-b border-border flex flex-row items-center justify-between">
            <div>
              <CardTitle className="text-base text-foreground">Fulfillment Allocation Matrix</CardTitle>
              <p className="text-xs text-muted-foreground mt-0.5">
                Use the inputs below to manually adjust unit allocations between warehouses.
              </p>
            </div>
            <Badge variant="outline" className="font-mono text-xs gap-1.5 border-pencil-gray/40 text-forest-ink/60">
              <Sliders className="h-3 w-3 text-forest-ink/60" />
          ...
          <CardContent className="p-0">
            {allocations.length === 0 ? (
              ...
              <div className="divide-y divide-pencil-gray/20">
                {allocations.map((allocation, index) => (
                  <div
                    key={`${allocation.productId}-${allocation.warehouseId}-${index}`}
                    className="grid gap-4 p-5 md:grid-cols-[2fr_1.5fr_1.5fr_1fr] md:items-center hover:bg-whisper-gray/50"
                  >
                    <div>
                      <p className="text-sm font-semibold text-foreground">{allocation.productName}</p>
                      <p className="text-xs text-muted-foreground">
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="font-mono text-sm font-medium text-foreground">{allocation.quantityRequested} units</p>
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      {allocation.quantityBackordered > 0 ? (
                        ...
                        <Badge className="bg-sticky-note-mint text-forest-ink border-sticky-note-mint/60 text-[10px]">
                          Fully Allocated
                        </Badge>
                      )}
                    </div>
        ...
        <div className="flex items-center justify-between pt-2">
          {!canConfirm ? (
            <span className="text-xs text-muted-foreground">
              Note: Switch to <span className="font-semibold text-forest-ink">Sales Manager</span> or <span className="font-semibold text-forest-ink">Admin</span> role to confirm allocations.
            </span>
          ) : (
            <span className="text-xs text-muted-foreground">
          ...
          <Button
            ...
            className="gap-2 bg-forest-ink text-cream-paper"
          >
